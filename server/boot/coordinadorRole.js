...
module.exports = function(app) {
  ...
  Role.registerResolver('coordinadorJuego', function(role, context, cb) {
    ...
    context.model.findById(context.modelId, function(err, instancia) {
      ...
      if(!instancia) {
        let error = new Error("Instancia no encontrada");
          error.statusCode = 404;
        return cb(error);
      }

      instancia.juegoAlQuePertenece(function(err, juego){
          if(err) return cb(err);
            ...
            // (using count() because we only want to know if such a record exists)
                juego.coordinadores.findById(userId,
                        function (err, usuario) {
                            if(err) return cb(null, false);
                            
                            if(usuario) return cb(null, true);
